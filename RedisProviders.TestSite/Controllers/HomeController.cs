...
namespace RedisProviders.TestSite.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            // Read the previous values stored in the session state store.
            ViewBag.PreviousTime = Session["Time"];
            ViewBag.Text = Session["Text"];
            

            // store some objects in session state
            Session["Time"] = DateTime.UtcNow;
            Session["Text"] = LONG_STRING;
            Session["RequestNumber"] = int.Parse((Session["RequestNumber"] ?? 0).ToString()) + 1;

            // set some values for display
            ViewBag.Time = Session["Time"];
            ViewBag.RequestNumber = Session["RequestNumber"];
